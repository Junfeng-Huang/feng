...
// NewFengEnv 有一个参数，.env文件所在的目录
// example: NewFengEnv("/envfolder/") 会读取文件: /envfolder/.env
// .env的文件格式 FOO_ENV=BAR
func NewFengEnv(params ...interface{}) (interface{}, error) {
	if len(params) != 1 {
		return nil, errors.New("NewFengEnv param error")
	}
...
func (en *FengEnv) AppEnv() string {
	return en.Get("APP_ENV")
}

// IsExist 判断一个环境变量是否有被设置
func (en *FengEnv) IsExist(key string) bool {
	_, ok := en.maps[key]
	return ok
}

// Get 获取某个环境变量，如果没有设置，返回""
func (en *FengEnv) Get(key string) string {
...
func (en *FengEnv) All() map[string]string {
	return en.maps
}
