...
// FengConfig  表示feng框架的配置文件服务
type FengConfig struct {
...
// 读取某个配置文件
func (conf *FengConfig) loadConfigFile(folder string, file string) error {
	conf.lock.Lock()
...
// 删除文件的操作
func (conf *FengConfig) removeConfigFile(folder string, file string) error {
	conf.lock.Lock()
	defer conf.lock.Unlock()
	s := strings.Split(file, ".")
	// 只有yaml或者yml后缀才执行
...
// NewFengConfig 初始化Config方法
func NewFengConfig(params ...interface{}) (interface{}, error) {
	...
	if err != nil {
		return nil, err // TODO:使用github.com/pkg/errors的WithStack函数打印会更好
	}
	for _, file := range files {
		fileName := file.Name()
		err := fengConf.loadConfigFile(envFolder, fileName)
		if err != nil {
			log.Println(err)
			continue
		}
			...
			case ev := <-watch.Events:
				{
					...
					if ev.Op&fsnotify.Create == fsnotify.Create {
						...
						fengConf.loadConfigFile(folder, fileName)
					}
					...
					if ev.Op&fsnotify.Remove == fsnotify.Remove {
						...
						fengConf.removeConfigFile(folder, fileName)
					}
				}
...
// 通过path来获取某个配置项
func (conf *FengConfig) find(key string) interface{} {
	conf.lock.RLock()
